...
impl Expr {
    ...
    pub fn parser() -> impl Parser<char, Self, Error = Simple<char>> {
        recursive(|expr| {
            ...
            let binary = cmp.clone().then(cmpop!("&&", Expr::And; "||", Expr::Or).then(cmp).repeated()).foldl(|l, (op, r)| op(Box::new(l), Box::new(r)));
            binary
                .clone()
                .then_ignore(op('?'))
                .then(binary.clone())
                .then_ignore(op(':'))
                .then(binary.clone())
                .map(|((cond, then), other)| Expr::Ter(Box::new(cond), Box::new(then), Box::new(other)))
                .or(binary)
        })
        .then_ignore(end())
    }
